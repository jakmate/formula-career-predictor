...
async def train_models_task():
    ...
    try:
        ...
        global ml_models, deep_models, feature_cols, scaler
        ml_models, deep_models, _, _, feature_cols, scaler = train_models(features_df)

        # Update system status
        system_status["last_training"] = datetime.now()
        system_status["models_available"] = list(ml_models.keys()) + list(deep_models.keys())
...
async def list_models():
    ...
    return {
        ...
        "total_models": len(ml_models) + len(deep_models)
    }
...
async def get_predictions(model_name: str) -> ModelResults:
    ...
    if model_name not in ml_models and model_name not in deep_models:
...
async def get_all_predictions():
    ...
    for model_name in list(ml_models.keys()) + list(deep_models.keys()):
        try:
