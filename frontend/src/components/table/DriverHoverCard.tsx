...
export const DriverHoverCard = ({ driver, children }: DriverHoverCardProps) => {
  ...
  const handleMouseEnter = () => {
    ...
    if (triggerRef.current) {
      ...
      setPosition(spaceBelow < cardHeight && spaceAbove > spaceBelow ? 'top' : 'bottom');
    }
  ...
  const driverInfo = {
    ...
    photo: `https://ui-avatars.com/api/?name=${driver.driver.split(' ').map(n => n[0]).join('')}&background=1e40af&color=fff`
  };
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                <h3 className="text-white font-semibold text-lg">{driverInfo.fullName}</h3>
