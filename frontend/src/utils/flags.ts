  ...
  return React.createElement(
    ...
    flags.map((FlagComponent, index) => 
      React.createElement(
        ...
        React.createElement(FlagComponent, { className: "w-full h-full object-cover" })
      )
    )
  );
};

export const CountryFlag: React.FC<CountryFlagProps> = ({ nationality, className = "w-6 h-4" }) => {
